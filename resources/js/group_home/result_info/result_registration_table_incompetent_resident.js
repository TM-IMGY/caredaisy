/**
 * 実績登録テーブル(特定入所者サービス)に責任を持つクラス。
 * サービスのテーブルと共通項も多いが考え方が異なるので切り離した。
 */
export default class ResultRegistrationTableIncompetentResident{
  constructor(){
    this.elementTbody = document.getElementById('result_registration_tbody_incompetent_resident');
    this.elementDateCell = document.getElementById('rrt_date_incompetent_resident');
    this.elementDowCell = document.getElementById('rrt_dow_incompetent_resident');
  }

  /**
   * レコードを追加する。
   * @param {Number} burdenLimit 
   ...
   */
  addRecord(burdenLimit, dateDailyRate, serviceCountDate, serviceItemCodeId, serviceItemName, targetDate, unitNumber){
    ...
    // 実績ラベルに特定入所者サービスコードIDと負担者限度額をセットする。
    resultLbl.setAttribute('data-rrt-incompetent-resident-id', serviceItemCodeId);
    resultLbl.setAttribute('data-rrt-incompetent-payer-limit', burdenLimit);
    ...
    // 作成したセルをダミーの行に追加する。
    let dummyRecordCells = [trash, serviceItemNameCell, unitNumberCell];
    ...
    return records
      ...
      .map(record => {
        ...
        return {
          ...
          // 0は実績ラベルセルのインデックス。
          burden_limit: Number(cells[0].getAttribute('data-rrt-incompetent-payer-limit')),
          service_item_code_id: Number(cells[0].getAttribute('data-rrt-incompetent-resident-id')),
  ...
  reloadDateCell(year, month, stayOutDays, startDates, endDates){
    ...
    // 日付セルを全て削除する。
    let dateCells = Array.from(document.getElementsByClassName('rrt_header_date_incompetent_resident'));
    ...
    for (let i = dateCnt; i > 0; i--) {
      let cell = document.createElement('td');
      cell.textContent = i;
      cell.className = 'result_registration_table_cell result_registration_table_header rrt_header_date_incompetent_resident';
    ...
    // 曜日セルを削除する。
    let dowCells = Array.from(document.getElementsByClassName('rrt_header_dow_incompetent_resident'));
    ...
    for (let i = dateCnt; i > 0; i--) {
      ...
      cell.className = 'result_registration_table_cell result_registration_table_header rrt_header_dow_incompetent_resident';
      if(day == 0){
    ...
    document.getElementById('rrt_ddr_incompetent_resident').colSpan = dateCnt + 2;
  }
}
