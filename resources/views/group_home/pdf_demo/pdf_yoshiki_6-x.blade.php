...
<head>
  ...
  <link media="all" rel='stylesheet' href="{{ asset('/css/group_home/pdf_demo/pdf_yoshiki_6-x.css') }}">
</head>
<body>
  @for ($s = 0; $s < count($data['benefit_status']); $s++)
    <div class="meisai">
      {{-- サービス種別が33または36の場合 --}}
      @if($data['service_type_code'] == \App\Models\ServiceType::SERVICE_TYPE_CODE_SPECIFIC_FACILITY_RESIDENT_CARE
        || $data['service_type_code'] == \App\Models\ServiceType::SERVICE_TYPE_CODE_COMMUNITY_BASED_SPECIFIC_FACILITY_RESIDENT_LIFE_CARE
      )
      ...
      <div>
        ...
        {{-- 公費受給者番号 --}}
        <div class="recipient_number_6x">{{ $data['user_public_expense_information']['recipient_number'] }}</div>
      </div>

      {{-- 提出年月 --}}
      <div>
        <span class="yaers_6x">{{ $data['japanese_imperial_year']['year'] }}</span>
        <span class="months_6x">{{ $data['japanese_imperial_year']['month'] }}</span>
      </div>

      {{-- 保険者番号 --}}
      <div class="insurer_no_6x">{{ $data['facility_user']['insurer_no'] }}</div>

      {{-- 施設利用者情報 --}}
      <div>
        {{-- 被保険者番号 --}}
        <span class="insured_no_6x">{{ $data['facility_user']['insured_no'] }}</span>
        {{-- フリガナ --}}
        <div class="name_kana">
          {{-- フリガナ姓 --}}
      ...
      <div>
        ...
        <div class="care_level">
          ...
          {{-- 要支援1 --}}
          @elseif($data['user_care_information']['care_level_id'] == '4')
            <span class="care_level_12 service_type_35"></span>
          {{-- 要支援2 --}}
          @elseif($data['user_care_information']['care_level_id'] == '5')
      ...
      @if(reset($data['benefit_status']) == $data['benefit_status'][$s])
        ...
        <div>
          {{-- 外部利用型給付上限単位数 --}}
          <div class="classification_support_limit_units">{{ $data['benefit_billing']['classification_support_limit_units'] }}</div>
          {{-- 外部利用型上限管理対象単位数 --}}
          <div class="classification_support_benefit_limit_unit">{{ $data['benefit_billing']['classification_support_benefit_limit_unit'] }}</div>
          {{-- 外部利用型外給付単位数 --}}
          <div class="classification_support_benefit_unit">{{ $data['benefit_billing']['classification_support_benefit_unit'] }}</div>
          {{-- 給付単位数 --}}
          <div class="unit_rate">
            {{-- 保険分 --}}
          ...
          <div class="rate">
            {{-- 保険分 --}}
            <span class="benefit_rate_6x">{{ $data['benefit_billing']['benefit_rate'] }}</span>
            {{-- 公費分 --}}
            <span class="public_benefit_rate_6x">{{ $data['public_spending']['public_benefit_rate'] }}</span>
          </div>
          {{-- 請求額（円） --}}
